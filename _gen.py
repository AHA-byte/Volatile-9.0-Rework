...
def gen_wares():
    out = ['<?xml version="1.0" encoding="utf-8"?>',
           ...
           '         Per-category TS cost (Shield 13, Engine 11, Weapon 10, Hull 12)',
    ...
    for cat in CATEGORIES:
        ...
        for N in range(MAX_LEVEL + 1):
            ...
            pct_min = round((mn - 1.0) * 100)
            pct_max = round((mx - 1.0) * 100)
            rng_str = f"{pct_min:+d}% .. {pct_max:+d}%"
            desc    = f"{PACK_DESCRIPTIONS[cat]} Level {N} range: {rng_str}. Apply cost: {ts_cost} Tuning Software. Permanent once applied."
            base    = 15000 + N * 3000
            avg     = 20000 + N * 4000
            mx_p    = 30000 + N * 6000
      ...
      <cues>
        ...
        <cue name="AwardXP" instantiate="true">
          ...
          <actions>
            ...
            <!-- XP gain: 1 .. 5 + currentLevel. -->
            <set_value name="$gain" min="1" max="5 + $curLvl"/>
            ...
            <!-- Target XP to reach lv(curLvl+1) from curLvl = 10 + 5 * curLvl.
                 lv0->lv1: 10, lv1->lv2: 15, lv2->lv3: 20, ... lv39->lv40: 205. -->
            <set_value name="$threshold" exact="10 + 5 * $curLvl"/>
            ...
            <do_elseif value="$newXP ge $threshold">
              ...
              <set_value name="$excess"  exact="$newXP - $threshold"/>
